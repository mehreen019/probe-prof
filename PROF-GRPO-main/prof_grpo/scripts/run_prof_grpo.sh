...
experiment_name=${model}-${algorithm}-${data}-prm${prof_filter}-n${n}-Both-Mean-clip${clip_ratio_high}
...
CUDA_VISIBLE_DEVICES=$(IFS=,; echo "${GPUS[*]}") python3 -m prof_grpo_both.main_prof --config-name prof_trainer \
