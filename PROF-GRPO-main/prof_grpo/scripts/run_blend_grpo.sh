...
experiment_name=${model}-${algorithm}-${data}-prm${prof_filter}-n${n}-Blend-beta${beta}-clip${clip_ratio_high}
...
CUDA_VISIBLE_DEVICES=$(IFS=,; echo "${GPUS[*]}") python3 -m blend_grpo.main_prof --config-name prof_trainer \
