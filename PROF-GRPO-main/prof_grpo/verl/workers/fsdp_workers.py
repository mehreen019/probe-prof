...
class ProcessRewardModelWorker(Worker):
    ...
    def init_model(self):
        ...
        from verl.workers.reward_model import DataParallelProcessRewardModel

        self.reward_module = self._build_reward_ref_model_optimizer(config=self.config)
        ...
        self.rm = DataParallelProcessRewardModel(
            config=self.config,
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        data.meta_info["max_token_len"] = self.config.forward_max_token_len_per_gpu
        data.meta_info["use_dynamic_bsz"] = self.config.use_dynamic_bsz
        data.meta_info["n"] = self.config.rollout_n
        # perform forward computation
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data=data)
            q, metrics = self.rm.compute_rm_score(data=data)

            prompt_length = data.batch["prompts"].shape[-1]
            response_mask = data.batch["attention_mask"][:, prompt_length:]
            acc = data.batch["seq_reward"]

            output = DataProto.from_dict(tensors={"values": q}, meta_info={"metrics": metrics})
            output = self.ulysses_sharding_manager.postprocess_data(data=output)
