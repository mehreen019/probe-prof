...
def karmarkar_karp(seqlen_list: List[int], k_partitions: int, equal_size: bool):
    ...
    states_pq = []
    if equal_size:
        remainder = len(seqlen_list) % k_partitions
        if remainder != 0:
            # Drop the smallest 'remainder' items to make it divisible
            print(f"[seqlen_balancing] Dropping {remainder} smallest items to make {len(seqlen_list)} divisible by {k_partitions}.")
            sorted_seqlen_list = sorted_seqlen_list[remainder:]
        # Update seqlen_list to match the dropped items for downstream assertions
        seqlen_list = [x[0] for x in sorted_seqlen_list]
        assert len(sorted_seqlen_list) % k_partitions == 0, f"{len(sorted_seqlen_list)} % {k_partitions} != 0"
